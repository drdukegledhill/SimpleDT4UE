...
struct ContentView: View {
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            HStack {
                Circle()
                    .fill(treeClient.isConnected ? Color.green : Color.red)
                    .frame(width: 12, height: 12)
                Text(treeClient.isConnected ? "Connected" : "Disconnected")
                    .foregroundColor(treeClient.isConnected ? .green : .red)
            }
            .padding(.bottom, 4)
            Text(treeClient.isConnected ? "Status: Connected to server" : "Status: Not connected")
                .font(.subheadline)
                .foregroundColor(treeClient.isConnected ? .green : .red)
    ...
    func pixelButton(for idx: Int, isStar: Bool = false) -> some View {
        ...
        return Button(action: {
            // If the pixel is black, set to white and brightness 1.0
            if color == .black {
                pixelColors[idx] = (.white, 1.0)
                sendPixelUpdate(idx)
            }
            selectedPixel = idx
        }) {
